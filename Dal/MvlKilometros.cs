...
namespace Rivn.Dal
{
    ...
    public static class MvlKilometros
    {
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "Up");

            try {
                // Recuperamos todos los registros
                return DBRuts.Exec_DS(p_dbcAccess,
                                      "TNGS_Rivn..MVLKILOMETROS_UP",
                                      new DbParameter[] {
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "ChkVersion");

            try {
                // Verificamos el número de versión
                DBRuts.Exec_DS(p_dbcAccess,
                               "TNGS_Rivn..MVLKILOMETROS_CHKVERSION",
                               new DbParameter[] {
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "Search");

            try {
                // Recuperamos el registro de la clave
                return DBRuts.Exec_DS(p_dbcAccess,
                                      "TNGS_Rivn..MVLKILOMETROS_SEARCH",
                                      new DbParameter[] {
                                          p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
                                          p_dbcAccess.MakeParam("@mkm_fyh_fecha", p_dtFecha),
                                          p_dbcAccess.MakeParam("@onlyactive", (p_bOnlyActive ? 1 : 0))
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "FSearch");

            try {
                // Recuperamos los registro de la clave foranea
                return DBRuts.Exec_DS(p_dbcAccess,
                                      "TNGS_Rivn..MVLKILOMETROS_FSEARCH",
                                      new DbParameter[] {
                                          p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
                                          p_dbcAccess.MakeParam("@onlyactive", (p_bOnlyActive ? 1 : 0))
                                      },
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "Insert");

            try {
                // Insertamos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..MVLKILOMETROS_INSERT",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
                                       p_dbcAccess.MakeParam("@mkm_fyh_fecha", p_dtFecha),
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "Update");

            try {
                // Modificamos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..MVLKILOMETROS_UPDATE",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
                                       p_dbcAccess.MakeParam("@mkm_fyh_fecha", p_dtFecha),
                                       p_dbcAccess.MakeParam("@mkm_nro_km", p_iKm),
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "Delete");

            try {
                // Borramos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..MVLKILOMETROS_DELETE",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
                                       p_dbcAccess.MakeParam("@mkm_fyh_fecha", p_dtFecha),
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "FDelete");

            try {
                // Borramos los registro de la clave foranea
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..MVLKILOMETROS_FDELETE",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "Recall");

            try {
                // Borramos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..MVLKILOMETROS_RECALL",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
                                       p_dbcAccess.MakeParam("@mkm_fyh_fecha", p_dtFecha),
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "FRecall");

            try {
                // Borramos los registro de la clave foranea
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..MVLKILOMETROS_FRECALL",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
                                       p_dbcAccess.MakeParam("@instante", p_dtInstante),
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "Drop");

            try {
                // Borramos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..MVLKILOMETROS_DROP",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
                                       p_dbcAccess.MakeParam("@mkm_fyh_fecha", p_dtFecha),
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "FDrop");

            try {
                // Borramos los registro de la clave foranea
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..MVLKILOMETROS_FDROP",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
                                   }
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "Pack");

            try {
                // Borramos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..MVLKILOMETROS_PACK",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
                                   }
                                  );
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "FPack");

            try {
                // Borramos los registro de la clave foranea
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..MVLKILOMETROS_FPACK",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@mkm_ecd_patente", p_strPatente),
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
                                   }
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "RemoveAllPatente");
        ...
        /// <param name="p_strTabla">Nombre de la tabla a llenar</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        public static int getLastFiveMvlKmG(DBConn p_dbcAccess,
                                            string p_strPatente,
                                            ref DataSet p_dsResult,
                                            string p_strTabla,
                                            ref StatMsg p_smResult)
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "getLastFiveMvlKmG");

            try {
                return DBRuts.Exec_DS(p_dbcAccess,
                                      "TNGS_Rivn..MVLKILOMETROS_GETLASTFIVEMVLKMG",
                                      new DbParameter[] {
                                          p_dbcAccess.MakeParam("@patente", p_strPatente),
                                          p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
                                      },
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("MvlKilometros", "MakeGridCaption");
