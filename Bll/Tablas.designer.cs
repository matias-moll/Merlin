...
namespace Mrln.Bll
{
    ...
    public static partial class Tablas
    {
        ...
        //**********************************************************
        //
        // Funciones para la Tabla: Encargados
        // Usando ClaseDal        : Encargados
        //
        //**********************************************************
        ...
        /// <summary>
        /// Devuelve la grilla de la tabla: Encargados
        /// </summary>
        /// <param name="p_bOnlyActive">Indica si solo se analizan los registros activos</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        /// <returns>Lista-entidad: LEEncargados</returns>
        public static LEEncargados EncUpFull(bool p_bOnlyActive,
                                             ref StatMsg p_smResult)
        {
            // No hay errores aun
            DBConn l_dbcAccess= null;
            ...
            try {
                ...
                // Pedimos los registros de la tabla
                return EncUpfl(l_dbcAccess, p_bOnlyActive, ref p_smResult);
            }
            catch (Exception l_expData) {
                // Error en la operacion UpFull
                p_smResult.BllError(l_expData);
        ...
        /// <summary>
        /// Devuelve una entidad: EEncargado
        /// </summary>
        /// <param name="p_strCod">Cod</param>
        /// <param name="p_bOnlyActive">Indica si solo se analizan los registros activos</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        /// <returns>Entidad: EEncargado</returns>
        public static EEncargado EncGet(string p_strCod,
                                        bool p_bOnlyActive,
                                        ref StatMsg p_smResult)
        {
            // No hay errores aun
            ...
            p_strCod= EEncargado.FrmtCod(p_strCod);

            try {
                // Obtenemos una conexion
                l_dbcAccess= DBRuts.GetConection(Connections.Dat);

                // Pedimos la entidad: EEncargado
                return EncSrch(l_dbcAccess,
                               p_strCod,
                               p_bOnlyActive,
                               ref p_smResult);
            }
        ...
        public static string EncNextKey(ref StatMsg p_smResult)
        {
            // No hay errores aun
            DBConn l_dbcAccess= null;
        ...
        /// <param name="p_smResult">Estado final de la operacion</param>
        public static void EncSave(EEncargado p_entEncargado,
                                   ref StatMsg p_smResult)
        {
            // No hay errores aun
            DBConn l_dbcAccess= null;
            ...
            try {
                ...
                // Grabamos la entidad: EEncargado
                EncSSav(l_dbcAccess, p_entEncargado, ref p_smResult);
            }
            catch (Exception l_expData) {
                // Error en la operacion Save
                p_smResult.BllError(l_expData);
        ...
        /// <param name="p_strCod">Cod</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        public static void EncEnabled(bool p_bEnable,
                                      string p_strCod,
                                      int p_iFxdVersion,
                                      ref StatMsg p_smResult)
        {
            ...
            try {
                ...
                if (p_smResult.ICodeEs(BllCodes.KeyDsntFound)) {
                    // Error. La clave no existe
                    p_smResult.BllWarning("El ítem (Encargado) que intenta modificar no existe en el sistema.","");
                    return;
                }
                ...
                if (p_bEnable) {
                    // Hay que habilitar el registro
                    Dal.Encargados.Recall(l_dbcAccess,
                                          p_strCod,
                                          ref p_smResult);
                }
                else {
                    // Hay que deshabilitar el registro
                    Dal.Encargados.Delete(l_dbcAccess,
                                          p_strCod,
                                          ref p_smResult);
                }
            }
        ...
        /// <param name="p_iFxdVersion">Versión del registro a borrar</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        public static void EncRemove(string p_strCod,
                                     int p_iFxdVersion,
                                     ref StatMsg p_smResult)
        {
            // No hay errores aun
        ...
        /// </summary>
        /// <param name="p_smResult">Estado final de la operacion</param>
        public static void EncPurge(ref StatMsg p_smResult)
        {
            // No hay errores aun
            DBConn l_dbcAccess= null;
            try {
        ...
        /// <summary>
        /// Valida la integridad de una entidad: Encargado
        /// </summary>
        /// <param name="p_dbcAccess">Conexion a la base de datos</param>
        /// <param name="p_entEncargado">Entidad con los datos a validar</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        internal static void EncTInt(DBConn p_dbcAccess,
                                     ...
                                     ref StatMsg p_smResult)
        {
            ...
            // Llamamos a la funcion fija del usuario
            EncTInt_f(p_dbcAccess, p_entEncargado, ref p_smResult);
            if (p_smResult.NOk) return;
        }

        /// <summary>
        /// Verifica si existe en la tabla una entidad: EEncargado
        ///      Retorno: p_smResult.Stat= BllAvisos.KeyExists   - La clave existe
        ///               p_smResult.Stat= BllAvisos.KeyNotFound - La clave no existe
        ///               p_smResult.Stat= BllAvisos.KeyDisabled - La clave está deshabilitada
        /// </summary>
        ...
        /// <param name="p_strCod">Cod</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        internal static void EncVKey(DBConn p_dbcAccess,
                                     string p_strCod,
                                     ref StatMsg p_smResult)
        {
            // No hay errores aun
            ...
            Dal.Encargados.Search(p_dbcAccess,
                                  p_strCod,
                                  false,
                                  ref l_dsTemp,
                                  "Temporal",
        ...
        /// <param name="p_iFxdVersion">Número de version</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        internal static void EncVVer(DBConn p_dbcAccess,
                                     string p_strCod,
                                     int p_iFxdVersion,
                                     ref StatMsg p_smResult)
        {
            ...
            // Verificamos el número de versión
            Dal.Encargados.ChkVersion(p_dbcAccess,
                                      p_strCod,
                                      p_iFxdVersion,
                                      ref l_dsTemp,
                                      "Temporal",
            ...
            // El número de versión no coincide
            l_dsTemp.Dispose();
            p_smResult.BllWarning("El número de versión del registro del ítem (Encargado) no coincide.\r\nOperación cancelada.","");
        }
        #endregion
        ...
        /// <summary>
        /// Devuelve una Lista-entidad: LEEncargados
        /// </summary>
        /// <param name="p_dbcAccess">Conexion a la base de datos</param>
        /// <param name="p_bOnlyActive">Indica si solo se analizan los registros activos</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        /// <returns>Lista-entidad: LEEncargados</returns>
        internal static LEEncargados EncUpfl(DBConn p_dbcAccess,
                                              bool p_bOnlyActive,
                                              ref StatMsg p_smResult)
        {
            try {
                // Pedimos los registros de la tabla: Encargados
                DataSet l_dsTemp= new DataSet();

                Dal.Encargados.Up(p_dbcAccess, 
                                  p_bOnlyActive,
                                  ref l_dsTemp, "Temporal",
                                  ref p_smResult);
                if (p_smResult.NOk) return null;
                ...
                // Fijamos los captions de la grilla
                Dal.Encargados.MakeGridCaptions(ref l_dsTemp, "Temporal", ref p_smResult);

                // Contruimos la lista-entidad y la devolvemos (si vino algun registro)
                LEEncargados l_lentRet= new LEEncargados(l_dsTemp.Tables["Temporal"]);
                l_dsTemp.Dispose();
                return l_lentRet;
            }
            catch (Exception l_expData) {
        ...
        /// <param name="p_smResult">Estado final de la operacion</param>
        /// <returns>Entidad: EEncargado</returns>
        internal static EEncargado EncSrch(DBConn p_dbcAccess,
                                           string p_strCod,
                                           bool p_bOnlyActive,
                                           ref StatMsg p_smResult)
        {
            try {
                // Pedimos el registro de la tabla: Encargados
                DataSet l_dsTemp= new DataSet();
                ...
                // Constuimos la entidad y la devolvemos (si vino un registro)
                EEncargado l_entRet= null;

                if (l_dsTemp.Tables["Temporal"].Rows.Count == 1)
                    l_entRet= new EEncargado(l_dsTemp.Tables["Temporal"].Rows[0]);
        ...
        /// <param name="p_smResult">Estado final de la operacion</param>
        /// <returns>Próxima clave</returns>
        internal static string EncGetNK(DBConn p_dbcAccess,
                                        ref StatMsg p_smResult)
        {
            try {
                // Pedimos la clave máxima
                ...
                Dal.Encargados.GetMaxKey(p_dbcAccess,
                                         ref l_dsTemp,
                                         "Temporal",
                                         ref p_smResult);
                if (p_smResult.NOk) return "";
        ...
        /// <param name="p_entEncargado">Entidad con los datos a procesar</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        internal static void EncSSav(DBConn p_dbcAccess,
                                     EEncargado p_entEncargado,
                                     ref StatMsg p_smResult)
        {
            try {
                // Procesamos codigo fijo
                EncSave_f(p_dbcAccess, ref p_entEncargado, ref p_smResult);
                if (p_smResult.NOk) return;
                ...
                // Si es una entidad nueva
                if (p_entEncargado.EsNueva) {
                    // Es un alta. La clave no debe existir
                    if (!p_smResult.ICodeEs(BllCodes.KeyDsntFound)) {
                        // Error. La clave ya existe
                        p_smResult.BllWarning("El ítem (Encargado) que intenta agregar ya existe en el sistema.","");
                        return;
                    }

                    // Agregamos el registro
                    EncInsr(p_dbcAccess, p_entEncargado, ref p_smResult);
                    return;
                }
                ...
                if (!p_smResult.ICodeEs(BllCodes.KeyExists)) {
                    // Error. La clave no existe o no está habilitada
                    p_smResult.BllWarning("El ítem (Encargado) que intenta modificar no existe en el sistema o no está habilitado.","");
                    return;
                }
                ...
                EncVVer(p_dbcAccess, 
                        p_entEncargado.Cod,
                        p_entEncargado.FxdVersion,
                        ref p_smResult);
                if (p_smResult.NOk) return;

                // Actualizamos el registro
                EncUpdt(p_dbcAccess, p_entEncargado, ref p_smResult);
            }
            catch (Exception l_expData) {
                // Error en la operacion SSav
                p_smResult.BllError(l_expData);
        ...
        /// <summary>
        /// Agrega un registro a la tabla a partir de una entidad: EEncargado
        /// </summary>
        /// <param name="p_dbcAccess">Conexion a la base de datos</param>
        /// <param name="p_entEncargado">Entidad con los datos a procesar</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        internal static void EncInsr(DBConn p_dbcAccess,
                                     EEncargado p_entEncargado,
                                     ref StatMsg p_smResult)
        {
            try {
                // Validamos la integridad de la entidad
                EncTInt(p_dbcAccess, p_entEncargado, ref p_smResult);
                if (p_smResult.NOk) return;

                // Creamos un nuevo registro de la tabla: Encargados
                Dal.Encargados.Insert(p_dbcAccess,
                                      p_entEncargado.Cod,
                                      p_entEncargado.Des,
                                      ref p_smResult);
            }
        ...
        /// <summary>
        /// Actualiza un registro a la tabla a partir de una entidad: EEncargado
        /// </summary>
        /// <param name="p_dbcAccess">Conexion a la base de datos</param>
        /// <param name="p_entEncargado">Entidad con los datos a procesar</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        internal static void EncUpdt(DBConn p_dbcAccess,
                                     EEncargado p_entEncargado,
                                     ref StatMsg p_smResult)
        {
            try {
                ...
                // Actualizamos un registro de la tabla: Encargados
                Dal.Encargados.Update(p_dbcAccess,
                                      p_entEncargado.Cod,
                                      p_entEncargado.Des,
                                      ref p_smResult);
            }
        ...
        /// <summary>
        /// Borra físicamente un registro de a tabla: Encargados
        /// </summary>
        /// <param name="p_dbcAccess">Conexion a la base de datos</param>
        /// <param name="p_strCod">Cod</param>
        /// <param name="p_iFxdVersion">Versión del registro a borrar</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        internal static void EncDrop(DBConn p_dbcAccess,
                                     string p_strCod,
                                     int p_iFxdVersion,
                                     ref StatMsg p_smResult)
        {
            try {
                ...
                if (p_smResult.ICodeEs(BllCodes.KeyDsntFound)) {
                    // Error. La clave no existe
                    p_smResult.BllWarning("El ítem (Encargado) que intenta borrar no existe en el sistema.","");
                    return;
                }
                ...
                // Borramos físicamente el registro
                Dal.Encargados.Drop(p_dbcAccess,
                                    p_strCod,
                                    ref p_smResult);
            }
            catch (Exception l_expData) {
        ...
        /// <param name="p_dbcAccess">Conexion a la base de datos</param>
        /// <param name="p_smResult">Estado final de la operacion</param>
        internal static void EncPack(DBConn p_dbcAccess,
                                     ref StatMsg p_smResult)
        {
            try {
                // Borramos los borrados lógicamente
                Dal.Encargados.Pack(p_dbcAccess,
                                    ref p_smResult);
            }
            catch (Exception l_expData) {
                // Error en la operacion Updt
                                      ...
                                      ref StatMsg p_smResult)
        {
            ...
            try {
                ...
                if (p_smResult.ICodeEs(BllCodes.KeyDsntFound)) {
                    // Error. La clave no existe
                    p_smResult.BllWarning("El ítem (MotivoInfraccion) que intenta modificar no existe en el sistema.","");
                    return;
                }
                                     ...
                                     ref StatMsg p_smResult)
        {
            ...
            // El número de versión no coincide
            l_dsTemp.Dispose();
            p_smResult.BllWarning("El número de versión del registro del ítem (MotivoInfraccion) no coincide.\r\nOperación cancelada.","");
        }
        #endregion
                                     ...
                                     ref StatMsg p_smResult)
        {
            try {
                ...
                if (p_entMotivoInfraccion.EsNueva) {
                    ...
                    if (!p_smResult.ICodeEs(BllCodes.KeyDsntFound)) {
                        // Error. La clave ya existe
                        p_smResult.BllWarning("El ítem (MotivoInfraccion) que intenta agregar ya existe en el sistema.","");
                        return;
                    }
                ...
                if (!p_smResult.ICodeEs(BllCodes.KeyExists)) {
                    // Error. La clave no existe o no está habilitada
                    p_smResult.BllWarning("El ítem (MotivoInfraccion) que intenta modificar no existe en el sistema o no está habilitado.","");
                    return;
                }
        ...
        /// <summary>
        /// Borra físicamente un registro de a tabla: MotivosInfracciones
        /// </summary>
        /// <param name="p_dbcAccess">Conexion a la base de datos</param>
        /// <param name="p_strCodigo">Código</param>
        /// <param name="p_iFxdVersion">Versión del registro a borrar</param>
                                     ...
                                     ref StatMsg p_smResult)
        {
            try {
                ...
                if (p_smResult.ICodeEs(BllCodes.KeyDsntFound)) {
                    // Error. La clave no existe
                    p_smResult.BllWarning("El ítem (MotivoInfraccion) que intenta borrar no existe en el sistema.","");
                    return;
                }
