...
namespace Mrln.Tg
{
    ...
    //----------------------------------------------------------------------------
    // Fecha                     : 06/06/2018 02:35
    // Sistema                   : Mrln
    // Interface para la Entidad : Taller
    ...
    /// <summary>
    /// Formulario para Mantenimiento de la Tabla:Taller
    /// </summary>
    public partial class Talleres : DockContent
    {
        #region Miembros de la Clase
            private Bel.ETaller m_entTaller= null;
        ...
        /// </summary>
        private void Talleres_Load(object sender, System.EventArgs e)
        {
        ...
        /// </summary>
        private void Talleres_FormClosed(object sender, FormClosedEventArgs e)
        {
            // Liberamos el menu
            App.LockMenu(false);
        }
        ...
        private void grdDatos_HeaderClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left) {
                ...
                if (m_strSort != grdDatos.GridOrder) {
                    ...
                    App.SetStrURegistry(false, "GridFormat", "TalleresGrdSort", m_strSort);
                    return;
                }
            }
        ...
        private void GrdColumn_WidthChanged(object sender, EventArgs e)
        {
            ...
            App.SetStrURegistry(false, "GridFormat", "TalleresGrdWidths", grdDatos.ColWitdhs);
        }
        ...
        private void cmdExcel_Click(object sender, System.EventArgs e)
        {
            ...
            grdDatos.ExportToExcel(false, false, "", "Talleres", ref m_smResult);
            App.EndAdvance();
            App.HideMsg();
        }
        ...
        private void cmdNuevo_Click(object sender, System.EventArgs e)
        {
            ...
            // damos foco al primer campo
            m_entTaller= Bel.ETaller.NewEmpty();
            m_entTaller.Codigo = Bll.Talleres.NextKey(ref m_smResult);
            m_smResult.Reset();
        ...
        private void cmdModificar_Click(object sender, System.EventArgs e)
        {
            ...
            m_entTaller= Bll.Talleres.Get((string) grdDatos.GetMatrixValueObj(l_iRow, 1),
        ...
        private void cmdPurgar_Click(object sender, EventArgs e)
        {
            ...
            if (MsgRuts.AskUser(this, "Atención!!!!\r\n" +
                                      ...
                                      /*App.Usuario.Usuario +*/ "TalleresPurge") == DialogResult.No) return;

            // Purgamos la tabla
            App.ShowMsg("Compactando la tabla...");
            Bll.Talleres.Purge(ref m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void cmdDesHab_Click(object sender, System.EventArgs e)
        {
            ...
            Bll.Talleres.Enabled(m_entTaller.EstaBorrada,
                                 m_entTaller.Codigo,
                                 m_entTaller.FxdVersion,
                                 ref m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void cmdGrabar_Click(object sender, System.EventArgs e)
        {
            ...
            m_entTaller.Codigo= txtCodigo.Text;
            m_entTaller.Descripcion= txtDescripcion.Text;
            m_entTaller.Direccion= txtDireccion.Text;
            m_entTaller.Telefono= txtTelefono.Text;
            m_entTaller.Contacto= txtContacto.Text;

            // Tratamos de grabar la entidad
            App.ShowMsg("Grabando...");
            Bll.Talleres.Save(m_entTaller, ref m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void FillGrid()
        {
            ...
            App.ShowMsg("Recuperando datos...");
            Bel.LETalleres l_lentTalleres= Bll.Talleres.UpFull(false, ref m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void EditMode()
        {
            ...
            txtCodigo.Text= m_entTaller.Codigo;
            txtDescripcion.Text= m_entTaller.Descripcion;
            txtDireccion.Text= m_entTaller.Direccion;
            txtTelefono.Text= m_entTaller.Telefono;
            txtContacto.Text= m_entTaller.Contacto;

            // Habilitamos el frame
            txtCodigo.NormalDisable= false;
            txtCodigo.Enabled= false;
            txtDescripcion.NormalDisable= false;
            txtDescripcion.Enabled= m_entTaller.EsNueva;
            txtDireccion.NormalDisable= false;
            txtDireccion.Enabled= !m_entTaller.EstaBorrada;
            txtTelefono.NormalDisable= false;
            txtTelefono.Enabled= !m_entTaller.EstaBorrada;
            txtContacto.NormalDisable= false;
            txtContacto.Enabled= !m_entTaller.EstaBorrada;
            cmdCancelar.Enabled= true;
            cmdGrabar.Enabled= !m_entTaller.EstaBorrada;
            cmdDesHab.Enabled= ((!m_entTaller.EsNueva) &&(!m_entTaller.EstaBorrada));
