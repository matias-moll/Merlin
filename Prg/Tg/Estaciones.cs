...
namespace Mrln.Tg
{
    ...
    //----------------------------------------------------------------------------
    // Fecha                     : 21/10/2013 16:53
    // Sistema                   : Mrln
    // Interface para la Entidad : Estacion
    ...
    /// <summary>
    /// Formulario para Mantenimiento de la Tabla:Estaciones
    /// </summary>
    public partial class Estaciones : DockContent
    {
        #region Miembros de la Clase
            private Bel.EEstacion m_entEstacion= null;
        ...
        /// </summary>
        private void Estaciones_Load(object sender, System.EventArgs e)
        {
        ...
        /// </summary>
        private void Estaciones_FormClosed(object sender, FormClosedEventArgs e)
        {
            // Liberamos el menu
            App.LockMenu(false);
        }
        ...
        private void grdDatos_HeaderClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left) {
                ...
                if (m_strSort != grdDatos.GridOrder) {
                    ...
                    App.SetStrURegistry(false, "GridFormat", "EstacionesGrdSort", m_strSort);
                    return;
                }
            }
        ...
        private void GrdColumn_WidthChanged(object sender, EventArgs e)
        {
            ...
            App.SetStrURegistry(false, "GridFormat", "EstacionesGrdWidths", grdDatos.ColWitdhs);
        }
        ...
        private void gbExcel_Click(object sender, System.EventArgs e)
        {
            ...
            grdDatos.ExportToExcel(false, false, "", "Estaciones", m_smResult);
            App.EndAdvance();
            App.HideMsg();
        }
        ...
        private void gbNuevo_Click(object sender, System.EventArgs e)
        {
            ...
            m_entEstacion.Cod = Bll.Tablas.EstNextKey(m_smResult);
            m_smResult.Reset();
        ...
        private void gbModificar_Click(object sender, System.EventArgs e)
        {
            ...
            m_entEstacion= Bll.Tablas.EstGet((string) grdDatos.GetMatrixValueObj(l_iRow, 1),
                                             false, m_smResult);
        ...
        private void gbCompactar_Click(object sender, EventArgs e)
        {
            ...
            if (MsgRuts.AskUser(this, "Atención!!!!\r\n" +
                                      ...
                                      /*App.Usuario.Usuario +*/ "EstacionesPurge") == DialogResult.No) return;

            // Purgamos la tabla
            App.ShowMsg("Compactando la tabla...");
            Bll.Tablas.EstPurge(m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void gbChangeHabilitado_Click(object sender, System.EventArgs e)
        {
            ...
            Bll.Tablas.EstEnabled(m_entEstacion.EstaBorrada,
                                  m_entEstacion.Cod,
                                  m_entEstacion.FxdVersion,
                                  m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void gbGrabar_Click(object sender, System.EventArgs e)
        {
            ...
            m_entEstacion.Cod= txtCod.Text;
            m_entEstacion.Des= txtDes.Text;
            m_entEstacion.Domicilio= txtDomicilio.Text;
            m_entEstacion.Contacto= txtContacto.Text;
            m_entEstacion.Telefono= txtTelefono.Text;

            // Tratamos de grabar la entidad
            App.ShowMsg("Grabando...");
            Bll.Tablas.EstSave(m_entEstacion, m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void FillGrid()
        {
            ...
            App.ShowMsg("Recuperando datos...");
            Bel.LEEstaciones l_lentEstaciones= Bll.Tablas.EstUpFull(false, m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;

            // Asignamos a la grilla
            App.InitAdvance("Cargando:");
            grdDatos.Focus();
            grdDatos.FillFromLEntidad(l_lentEstaciones, "deleted");
            grdDatos.ColWitdhs= App.GetStrURegistry(false, "GridFormat", "EstacionesGrdWidths", "");
            grdDatos.GridOrder= App.GetStrURegistry(false, "GridFormat", "EstacionesGrdSort", "");
            App.EndAdvance();

            // Fijamos el evento de cambio de ancho de la grilla
            if (l_lentEstaciones.Count > 0)
        ...
        private void OperationMode()
        {
            ...
            txtDomicilio.NormalDisable= true;
            txtDomicilio.Enabled= false;
            txtContacto.NormalDisable= true;
            txtContacto.Enabled= false;
            txtTelefono.NormalDisable= true;
        ...
        private void EditMode()
        {
            ...
            txtCod.Text= m_entEstacion.Cod;
            txtDes.Text= m_entEstacion.Des;
            txtDomicilio.Text= m_entEstacion.Domicilio;
            txtContacto.Text= m_entEstacion.Contacto;
            ...
            txtDomicilio.Enabled= !m_entEstacion.EstaBorrada;
            txtContacto.NormalDisable= false;
            txtContacto.Enabled= !m_entEstacion.EstaBorrada;
            txtTelefono.NormalDisable= false;
            txtTelefono.Enabled= !m_entEstacion.EstaBorrada;
            cmdCancelar.Enabled= true;
            cmdGrabar.Enabled= !m_entEstacion.EstaBorrada;
            gbDeshabilitar.Enabled= ((!m_entEstacion.EsNueva) &&(!m_entEstacion.EstaBorrada));
