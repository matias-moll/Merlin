...
namespace Rivn.Dal
{
    ...
    /// <summary>
    /// Modulo DAL de Acceso a la tabla: Controles
    /// </summary>
    public static class Controles
    {
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "Up");

            try {
                // Recuperamos todos los registros
                return DBRuts.Exec_DS(p_dbcAccess,
                                      "TNGS_Rivn..CONTROLES_UP",
                                      new DbParameter[] {
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "ChkVersion");

            try {
                // Verificamos el número de versión
                DBRuts.Exec_DS(p_dbcAccess,
                               "TNGS_Rivn..CONTROLES_CHKVERSION",
                               new DbParameter[] {
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "Search");

            try {
                // Recuperamos el registro de la clave
                return DBRuts.Exec_DS(p_dbcAccess,
                                      "TNGS_Rivn..CONTROLES_SEARCH",
                                      new DbParameter[] {
                                          p_dbcAccess.MakeParam("@ctl_cod_cod", p_strCod),
                                          p_dbcAccess.MakeParam("@onlyactive", (p_bOnlyActive ? 1 : 0))
                                      },
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "GetMaxKey");

            try {
                // Recuperamos la clave mas alta de la tabla
                DBRuts.Exec_DS(p_dbcAccess,
                               "TNGS_Rivn..CONTROLES_GETMAXKEY",
                               new DbParameter[] {
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "Insert");

            try {
                // Insertamos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..CONTROLES_INSERT",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@ctl_cod_cod", p_strCod),
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "Update");

            try {
                // Modificamos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..CONTROLES_UPDATE",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@ctl_cod_cod", p_strCod),
                                       p_dbcAccess.MakeParam("@ctl_des_des", p_strDes),
                                       p_dbcAccess.MakeParam("@ctl_cd1_crit", p_strCrit),
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "Delete");

            try {
                // Borramos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..CONTROLES_DELETE",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@ctl_cod_cod", p_strCod),
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
                                   }
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "Recall");

            try {
                // Borramos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..CONTROLES_RECALL",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@ctl_cod_cod", p_strCod),
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
                                   }
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "Drop");

            try {
                // Borramos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..CONTROLES_DROP",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@ctl_cod_cod", p_strCod),
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
                                   }
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "Pack");

            try {
                // Borramos el registro
                return DBRuts.Exec(p_dbcAccess,
                                   "TNGS_Rivn..CONTROLES_PACK",
                                   new DbParameter[] {
                                       p_dbcAccess.MakeParam("@usuario", DBConn.Usuario)
                                   }
                                  );
        ...
        {
            // No hay errores aun
            p_smResult.DalReset("Controles", "MakeGridCaption");
